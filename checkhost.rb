...
def help_output
  puts <<-EOF
--help
--target, -t <target_environment>
--noop, -n : do not update the datastores
--debug, -d
Example:
--target 0_1_3
  EOF
...
mc.discover()
debug("Discovered nodes: #{mc.pick_nodes_from_discovered(100000).inspect}")
mc.check(:environment => targetenv) do |resp|

  if resp[:body][:statuscode] != 0
    debug("#{resp[:senderid]}statuscode !=0: #{ resp[:body][:data].to_yaml}\n\n")
    next
  end
  ...
  begin
    ...
    begin
      ...
      debug("host #{resp[:senderid]}")
      debug("No json? #{resp[:body][:data][:output]}")
      output=[]
    end
    ...
    output.each do |event|
      ...
      event.delete("tags")
      event.delete("time")
      debug("Node: #{resp[:senderid]} -> Event=#{event.inspect}\n")
